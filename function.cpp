...
double read_double(lru_for_elements &cache, unsigned int ind) {
    double result = 0;
    uint8_t buffer[8];
    for (int i = 0; i < 8; i++)
...
void write_double(lru_for_elements &cache, unsigned int ind, double new_val) {
    uint8_t buffer[8];
...
double read_element_field(lru_for_elements &cache, unsigned int ind, const std::string& field_name) {
...
void write_element_field(lru_for_elements &cache, unsigned int ind, const std::string& field_name, double new_val) {
    std::string element_field[8] = {"x", "y", "ax", "ay", "vx", "vy", "a", "b"};
    for(int i = 0; i < 8; i++)
        if(element_field[i] == field_name)
...
void f(lru_for_elements &cache, int n, double asqr) {
    for (int i = 0; i < n; ++i) {
